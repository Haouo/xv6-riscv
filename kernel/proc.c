...
struct proc proc[NPROC]; // NOTE: this is shared by all harts
...
found:
    ...
    p->context.ra = (uint64)forkret; // WARN: returns to forkret() when it calls RET
...
// Set up first user process. (which is *initcode[]* on the above)
void userinit(void) {
    struct proc *p;
    ...
    // WARN: initialize page table of the first user proc.
    // NOTE: allocate one user page and copy initcode's instructions
    // and data into it.
